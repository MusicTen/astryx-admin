...
test('非 2xx 抛 ApiError（含 code/message）', async () => {
  const err = await http.get('forbidden').json().catch((e: unknown) => e);
...
test('401 清空登录态', async () => {
  useAuthStore.getState().login('tok', { id: '1', name: 'a', email: 'a@b.c' });
  await http.get('expired').json().catch(() => undefined);
  expect(useAuthStore.getState().token).toBeNull();
});
```
...
const CARDS: { key: keyof DashboardStats; title: string; variant: 'blue' | 'green' | 'purple' | 'red' }[] = [
